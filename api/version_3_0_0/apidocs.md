...
>*updates*:
>1. Добавлены атрибуты 'age', 'price', 'duration', 'genre' в модель event
>2. Изменены API №13, 14, 15, 16
...
|----------------	|-------------------	|
| request method 	| POST |
| route          	| BASE_URL/spectacles/{spectacle_id}/halls/{hall_id}/events|
...
|----------------	|-------------------	|
| request method 	| GET |
| route          	| BASE_URL/events/{event_name}/from={date_range_start}/to={date_range_end}/genre={event_genre}/age_from={event_age_start}/age_to={event_age_end}/price_from={event_price_start}/price_to={event_price_end}/duration_from={event_duration_start}/duration_to={event_duration_end}|
...
|----------------	|-------------------	|
| request method 	| GET |
| route          	| BASE_URL/events/is_choosen_for_main_page=1|
| error types    	|  |

#### RESPONSE DATA [SUCCESS]

>*Note*: 1) Возвращаются все события, чей параметр is_choosen_for_main_page == 1. В примере ответа от сервера приведено 2 результата. 2) Если таких событий нет, соответственно, premieres будет пустым списком.
